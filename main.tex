...
\end{equation*}
More precisely, using sequences, $x \circ_i y$ is obtained by replacing the $i$-th value of $x$ with the sequence obtained by adding $i-1$ to the values of $y$, and increasing by $s-1$ the values of $x$ greater than $s$. 
...
\end{equation*}
The element $x \circ_i y \in \X(r+s-1)$ is represented, up to signs, as the sum over all possible collections of order indices of the sequence obtained in the following two steps: 1) replace in order the occurrences of $i$ in $x$ by the corresponding sequence in the associated splitting having its values shifted up by $i-1$, and 2) shift up by $s-1$ the values of $x$ greater than $i$.

In \comch, elements in this operad are modeled using the class \texttt{SurjectionElement}, for example 
\begin{verbatim}
...
The signs appearing in this constructions are determined by the attribute \texttt{convention} with possible values the strings \texttt{McClure-Smith} and \texttt{Berger-Fresse}.
